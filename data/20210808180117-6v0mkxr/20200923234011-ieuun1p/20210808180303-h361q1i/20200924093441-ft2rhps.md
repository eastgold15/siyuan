...
date: 2020-09-24T09:34:41Z
lastmod: 2024-07-02T22:20:31Z
---
...
> In SiYuan[^38], the only important core concept is Content block[^39]. The content block can be formed through the formatting format, so that we can organize our thoughts and knowledge at the block-level granularity, and it is also convenient for reading and outputting long content.
...
[^1]: # Please Start Here
    ...
    * Navigate in the content block[^2] and Window and tab[^10]
    ...
    * [Pay to support development](https://b3log.org/siyuan/en/pricing.html)
    ...
    ### Function Features[^35]

    Connect with third-party cloud storage[^21]

    Search asset content[^36]
...
[^3]: # Ref Content Block
    ...
    * Use `[[`​ to search only doc blocks[^9]
...
[^7]: # Content Block Type
    ...
    |![html-block-20220429115636-e80k6ty.svg](assets/html-block-20220429115636-e80k6ty.svg)|HTML block|Leaf block|
...
[^12]: # FAQ
    ...
    The data is saved in the workspace[^13] folder, in the workspace/data/ folder:

    * ​`assets`​ is used to save all inserted assets[^15]
    * ​`emojis`​ is used to save emoji images
    * ​`snippets`​ is used to save code snippets
    * ​`storage`​ is used to save query conditions, layouts and flashcards[^17], etc.
    * ​`templates`​ is used to save template snippets[^18]
    * ​`widgets`​ is used to save widgets[^19]
    * ​`plugins`​ is used to save plugins[^20]
    ...
    * The rest of the folders are the notebook folders created by the user, files with the suffix of `.sy`​​​​ in the notebook folder are used to save the document data, and the data format is JSON
    ...
    Although it does not support third-party sync disks, it supports Connect with third-party cloud storage[^21].
    ...
    * If it has not been configured correctly before (for example, the keys on multiple devices are inconsistent) or all devices are unavailable and the key string cannot be obtained, you can reset the key by following the steps below:
    ...
    Member&apos;s privileges[^23] can only be used after payment, please refer to [Pricing](https://b3log.org/siyuan/en/pricing.html).
...
[^13]: # Workspace
    ...
    * ​`data`​ is used to save data, please refer to here[^14] for the specific structure
...
[^14]: * ​`assets`​ is used to save all inserted assets[^15]
    * ​`emojis`​ is used to save emoji images
    * ​`snippets`​ is used to save code snippets
    * ​`storage`​ is used to save query conditions, layouts and flashcards[^17], etc.
    * ​`templates`​ is used to save template snippets[^18]
...
[^15]: # Assets
    ...
    * Use `file://`​ absolute path and link: Insert a picture or file into a hyperlink as a local absolute path, shortcut key <kbd>Ctrl</kbd>​+<kbd>Shift</kbd>​+<kbd>V</kbd>​, or hold down <kbd>Alt</kbd>​ and drag in
    ...
    In <kbd>Settings</kbd> - <kbd>Assets</kbd>, you can clean up unreferenced assets by one-click. If you need to retrieve files that have been deleted by mistake, please via Rollback[^16].
...
[^16]: ## Rollback through file history
...
[^19]: # Widgets
    ...
    Compared with plugins[^20], a widget is a kind of content block, which follows the indexing and rendering rules of the block. The widget is mainly oriented to content block extension, not to program function extension.
...
[^20]: # Plugins
    ...
    Compared with widgets[^19], the plugin is mainly for program function extension, not for content block extension.
...
[^21]: # Connect with third-party cloud storage
    ...
    * Local file system
    ...
    Synchronizing data through third-party cloud storage services is also end-to-end encrypted[^22], and third-party cloud storage service providers cannot obtain our plaintext data.
...
[^23]: # Member's privileges

    Membership privileges are divided into two parts:

    * Annual subscription Cloud Services[^24]
    * One-time payment Function Features[^35]
...
[^24]: # Cloud Services
    ...
    * Inbox[^33]
    ...
    ​#Note#​: Cloud service related limitations[^34].
...
[^26]: # Data sync
    ...
    After initializing the data repo key[^27], if all goes well, the synchronization is already configured. If you are not familiar with synchronization configuration, it is recommended to open the synchronization configuration wizard by clicking the synchronization button. It is easier to configure through the wizard.
...
[^29]: # Data backup
    ...
    Data backup refers to uploading Data snapshot[^30] to the cloud.

    ## How to use

    1. After initializing the data repo key[^27], if all goes well, the backup has been configured
    ...
    5. After the snapshot of the cloud backup is downloaded to the local, you can restore the data by Rollback data snapshot[^31]
...
[^37]: ## Is SiYuan open source?

    SiYuan is completely open source, and the complete source code can be found on GitHub.


[^38]: ## Origin of the name "SiYuan"
...
[^39]: # What is a Content Block
    ...
    We can name each content block, add aliases and memo. Naming and aliases are mainly used for anti-link mention [^40]search, and memo are used to record some inconvenient information in the content area.
    ...
    You can use Embed Content Block[^41] to search and summarize the naming and memo, such as listing the content blocks that contain the keyword `short` in the memo:
...
[^41]: # Embed Content Block

    ## Overview

    The embed content block is mainly used to summarize the required content blocks, use `{{` and `}}` to wrap SQL scripts on a single line: `{{ SELECT * FROM blocks WHERE content LIKE'%Keyword%' }}`, database table please refer to here[^42]. #Content Block/Embed#
...
[^42]: ## Table `blocks`
    ...
    |type|Content block type, please refer to here[^43]|
    |subtype|Content block subtype, please refer to here[^44]|
...
[^48]: # Data history
    ...
    * Data snapshot: partitioned by file, encrypted and compressed and saved in the `workspace/repo/` folder, unreadable by humans and cannot be manually restored
    ...
    In addition to local data history, data snapshots are also used for cloud Data sync[^26].
...
[^58]: # Import and Export
    ...
    If the image syntax (local relative path) like `![foo](images/bar.png)`​ is encountered during import, SiYuan will convert `images/bar.png`​ into assets[^15]. But if it is `.images/bar.png`​, it will not be converted, because the path files starting with `.`​ are hidden files, SiYuan will not process them.
...
[^60]: # Daily notes
    ...
    * Template path (optional): use this template as the initial content when creating the daily note
    ...
    Tips: Use in the specified template Embed Content Block[^41] to summarize to-do items or content blocks that need to be viewed daily.
...
[^63]: # Database
    ...
    * Multiple views can be added to the same database. All views share the same data, but filtering and sorting settings are independent of each other.
    ...
    |Book|Type|Author|
    | -------------------------------------| ---------------| -----------|
    |Twenty Thousand Leagues Under The Sea|Science fiction|Jules Verne|
    |The Three-Body Problem|Science fiction|Liu Cixin|
    |The Wandering Earth|Science fiction|Liu Cixin|
    ...
    Templates can access, calculate and render the values of other fields in the same piece of data through the syntax introduced in template snippets[^18]:
    ...
    * Use `.action{ index . "custom-xxx" }`​ to access content block custom attributes
    ...
    | -------| ----| --------| --------| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    |Vanessa|200|200|400|{   "Clothing": 200,   "Food": 200,   "Member": "Vanessa",   "Member_created": "2025-05-10T10:46:54.434663392+08:00",   "created": "2024-02-08T17:35:47+08:00",   "entryCreated": "2024-02-08T17:35:47.609+08:00",   "entryUpdated": "2024-02-08T17:36:40.686+08:00",   "id": "20240208173547-veq5qz2",   "updated": "2024-02-08T17:36:40+08:00" }|
    |Daniel|100|100|200|{   "Clothing": 100,   "Food": 100,   "Member": "Daniel",   "Member_created": "2025-05-10T10:46:54.435094519+08:00",   "created": "2024-02-08T17:36:40+08:00",   "entryCreated": "2024-02-08T17:36:40.748+08:00",   "entryUpdated": "2024-02-08T17:36:45.315+08:00",   "id": "20240208173640-vtvezcd",   "updated": "2024-02-08T17:36:45+08:00" }|
...
[^75]: # Mobile App
    ...
    * Please do not switch applications and keep the screen on during data synchronization, otherwise data synchronization will fail
    ...
    2. Synchronize it to the Android side through Data sync[^26]
...
[^76]: # Data Security
    ...
    * The availability of data depends on the availability of hardware disks and operating systems
    ...
    SiYuan cloud synchronization use end-to-end encrypted[^22], the password is stored locally with a built-in key encryption, any third party other than the user cannot get the plaintext in the cloud data.
...
[^77]: # Data Availability Guarantee
    ...
    * It is forbidden to synchronize data through third-party sync disk software to avoid file damage
    ...
    It is not enough to improve the availability of data on the local device, because the cloud Data sync[^26] is one of the rigid requirements. Data synchronization will affect local device data, so we should try our best to ensure the availability of data after synchronization:
...
[^79]: # Glossary
    ...
    * Export preview: preview after conversion to standard Markdown
    ...
    Through the service capabilities provided by the SiYuan kernel use on browser[^11].
...
[^83]: # Extend development
    ...
    * widgets[^19] Sample project github repo: siyuan-note/widget-sample
    * plugins[^20] Sample project github repo: siyuan-note/plugin-sample
    * template snippets[^18] Sample project github repo: siyuan-note/template-sample
