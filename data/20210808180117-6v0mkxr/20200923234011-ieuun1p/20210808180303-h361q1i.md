...
[^2]: # Please Start Here
    ...
    * Navigate in the content block[^3] and Window and tab[^11]
    ...
    * [Pay to support development](https://b3log.org/siyuan/en/pricing.html)
    ...
    In SiYuan[^39], the only important core concept is Content block[^40]. The content block can be formed through the formatting format, so that we can organize our thoughts and knowledge at the block-level granularity, and it is also convenient for reading and outputting long content.
    ...
    ### Function Features[^36]

    Connect with third-party cloud storage[^22]

    Search asset content[^37]
...
[^4]: # Ref Content Block
    ...
    * Use `((`​​ to search in all types of blocks[^8]
    * Use `[[`​ to search only doc blocks[^10]
...
[^13]: # FAQ
    ...
    The data is saved in the workspace[^14] folder, in the workspace/data/ folder:

    * ​`assets`​ is used to save all inserted assets[^16]
    * ​`emojis`​ is used to save emoji images
    * ​`snippets`​ is used to save code snippets
    * ​`storage`​ is used to save query conditions, layouts and flashcards[^18], etc.
    * ​`templates`​ is used to save template snippets[^19]
    * ​`widgets`​ is used to save widgets[^20]
    * ​`plugins`​ is used to save plugins[^21]
    ...
    * The rest of the folders are the notebook folders created by the user, files with the suffix of `.sy`​​​​ in the notebook folder are used to save the document data, and the data format is JSON
    ...
    Although it does not support third-party sync disks, it supports Connect with third-party cloud storage[^22].
    ...
    * If it has not been configured correctly before (for example, the keys on multiple devices are inconsistent) or all devices are unavailable and the key string cannot be obtained, you can reset the key by following the steps below:
    ...
    Member&apos;s privileges[^24] can only be used after payment, please refer to [Pricing](https://b3log.org/siyuan/en/pricing.html).
...
[^14]: # Workspace
    ...
    * ​`data`​ is used to save data, please refer to here[^15] for the specific structure
...
[^15]: * ​`assets`​ is used to save all inserted assets[^16]
    * ​`emojis`​ is used to save emoji images
    * ​`snippets`​ is used to save code snippets
    * ​`storage`​ is used to save query conditions, layouts and flashcards[^18], etc.
    * ​`templates`​ is used to save template snippets[^19]
...
[^16]: # Assets
    ...
    * Use `file://`​ absolute path and link: Insert a picture or file into a hyperlink as a local absolute path, shortcut key <kbd>Ctrl</kbd>​+<kbd>Shift</kbd>​+<kbd>V</kbd>​, or hold down <kbd>Alt</kbd>​ and drag in
    ...
    In <kbd>Settings</kbd> - <kbd>Assets</kbd>, you can clean up unreferenced assets by one-click. If you need to retrieve files that have been deleted by mistake, please via Rollback[^17].
...
[^17]: ## Rollback through file history
...
[^20]: # Widgets
    ...
    Compared with plugins[^21], a widget is a kind of content block, which follows the indexing and rendering rules of the block. The widget is mainly oriented to content block extension, not to program function extension.
...
[^21]: # Plugins
    ...
    Compared with widgets[^20], the plugin is mainly for program function extension, not for content block extension.
...
[^22]: # Connect with third-party cloud storage
    ...
    * Local file system
    ...
    Synchronizing data through third-party cloud storage services is also end-to-end encrypted[^23], and third-party cloud storage service providers cannot obtain our plaintext data.
...
[^24]: # Member's privileges

    Membership privileges are divided into two parts:

    * Annual subscription Cloud Services[^25]
    * One-time payment Function Features[^36]
...
[^25]: # Cloud Services
    ...
    * Inbox[^34]
    ...
    ​#Note#​: Cloud service related limitations[^35].
...
[^27]: # Data sync
    ...
    After initializing the data repo key[^28], if all goes well, the synchronization is already configured. If you are not familiar with synchronization configuration, it is recommended to open the synchronization configuration wizard by clicking the synchronization button. It is easier to configure through the wizard.
...
[^30]: # Data backup
    ...
    Data backup refers to uploading Data snapshot[^31] to the cloud.

    ## How to use

    1. After initializing the data repo key[^28], if all goes well, the backup has been configured
    ...
    5. After the snapshot of the cloud backup is downloaded to the local, you can restore the data by Rollback data snapshot[^32]
...
[^38]: ## Is SiYuan open source?

    SiYuan is completely open source, and the complete source code can be found on GitHub.


[^39]: ## Origin of the name "SiYuan"
...
[^40]: # What is a Content Block
    ...
    We can name each content block, add aliases and memo. Naming and aliases are mainly used for anti-link mention [^41]search, and memo are used to record some inconvenient information in the content area.
    ...
    You can use Embed Content Block[^42] to search and summarize the naming and memo, such as listing the content blocks that contain the keyword `short` in the memo:
...
[^42]: # Embed Content Block

    ## Overview

    The embed content block is mainly used to summarize the required content blocks, use `{{` and `}}` to wrap SQL scripts on a single line: `{{ SELECT * FROM blocks WHERE content LIKE'%Keyword%' }}`, database table please refer to here[^43]. #Content Block/Embed#
...
[^43]: ## Table `blocks`
    ...
    |type|Content block type, please refer to here[^44]|
    |subtype|Content block subtype, please refer to here[^45]|
...
[^49]: # Data history
    ...
    * Data snapshot: partitioned by file, encrypted and compressed and saved in the `workspace/repo/` folder, unreadable by humans and cannot be manually restored
    ...
    In addition to local data history, data snapshots are also used for cloud Data sync[^27].
...
[^59]: # Import and Export
    ...
    If the image syntax (local relative path) like `![foo](images/bar.png)`​ is encountered during import, SiYuan will convert `images/bar.png`​ into assets[^16]. But if it is `.images/bar.png`​, it will not be converted, because the path files starting with `.`​ are hidden files, SiYuan will not process them.
...
[^61]: # Daily notes
    ...
    * Template path (optional): use this template as the initial content when creating the daily note
    ...
    Tips: Use in the specified template Embed Content Block[^42] to summarize to-do items or content blocks that need to be viewed daily.
...
[^64]: # Database
    ...
    * Multiple views can be added to the same database. All views share the same data, but filtering and sorting settings are independent of each other.
    ...
    Templates can access, calculate and render the values of other fields in the same piece of data through the syntax introduced in template snippets[^19]:
    ...
    * Use `.action{ index . "custom-xxx" }`​ to access content block custom attributes
    ...
    | -------| ----| --------| --------| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    |Vanessa|200|200|400|{   "Clothing": 200,   "Food": 200,   "Member": "Vanessa",   "Member_created": "2025-05-10T10:46:50.025784416+08:00",   "created": "2024-02-08T17:35:47+08:00",   "entryCreated": "2024-02-08T17:35:47.609+08:00",   "entryUpdated": "2024-02-08T17:36:40.686+08:00",   "id": "20240208173547-veq5qz2",   "updated": "2024-02-08T17:36:40+08:00" }|
    |Daniel|100|100|200|{   "Clothing": 100,   "Food": 100,   "Member": "Daniel",   "Member_created": "2025-05-10T10:46:50.025424746+08:00",   "created": "2024-02-08T17:36:40+08:00",   "entryCreated": "2024-02-08T17:36:40.748+08:00",   "entryUpdated": "2024-02-08T17:36:45.315+08:00",   "id": "20240208173640-vtvezcd",   "updated": "2024-02-08T17:36:45+08:00" }|
...
[^76]: # Mobile App
    ...
    * Please do not switch applications and keep the screen on during data synchronization, otherwise data synchronization will fail
    ...
    2. Synchronize it to the Android side through Data sync[^27]
...
[^77]: # Data Security
    ...
    * The availability of data depends on the availability of hardware disks and operating systems
    ...
    SiYuan cloud synchronization use end-to-end encrypted[^23], the password is stored locally with a built-in key encryption, any third party other than the user cannot get the plaintext in the cloud data.
...
[^78]: # Data Availability Guarantee
    ...
    * It is forbidden to synchronize data through third-party sync disk software to avoid file damage
    ...
    It is not enough to improve the availability of data on the local device, because the cloud Data sync[^27] is one of the rigid requirements. Data synchronization will affect local device data, so we should try our best to ensure the availability of data after synchronization:
...
[^80]: # Glossary
    ...
    * Export preview: preview after conversion to standard Markdown
    ...
    Through the service capabilities provided by the SiYuan kernel use on browser[^12].
...
[^84]: # Extend development
    ...
    * widgets[^20] Sample project github repo: siyuan-note/widget-sample
    * plugins[^21] Sample project github repo: siyuan-note/plugin-sample
    * template snippets[^19] Sample project github repo: siyuan-note/template-sample
